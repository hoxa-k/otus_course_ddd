...
## ДЗ 9 "Эксплуатация и развертывание"

### 1. Обоснование микросервиса
Причины, по которым Orders стоит выделить в отдельный микросервис:
1. Один из самых высоконагруженных контекстов. Пользователи чаксто делают заказы, изменяются статусы заказов. Что бы была возможность независимо от других контекстов увеличивать его производительность (масштабировать) выделяем в отдельный микросервис.
2. В этом модуле часто происходят изменения. Добавляются новые бизнес-требования. Что бы была возможность независимого деплоя - выделяем в отдельный микросервис.
3. Как для одного из важных микросервисов для бизнеса и сложных в реализации, собираемся выделить отдельную команду для дальнейшей разработки, поэтому выделяем в отдельный микросервис.

### 2. Проектирование CI/CD Pipeline
1. CI
 - Checkout: загрузка исходного кода из Git-репозитория.
 - Lint и проверка качества: статический анализ кода (например, SonarQube).
 - Запуск unit тестов
 - Запуск интеграционных тестов.
 - Сборка артефакта: компиляция приложения и упаковка в исполняемый файл.
 - Сборка Docker-образа
2. CD
 - Загрузка Docker-образа
 - Деплой (Blue-Green Deployment, так как требуется бесшовное обновление и возможность мгновенного возврата)

### 3. Безопасная миграция БД (Expand-Contract)
**1 релиз.** Expand - Создаем новое поле в таблице Orders - order_state. Изменяем код репозитория (OrderRepository), что бы одновременно писать одинаковые данные в обе колонки status и order_state.

**2 релиз.** Migrate - Создаем скрипт, который в фоновом процессе синхронизирует старые данные (добавленные до 1-го релиза) между колонками status и order_state.

**3 релиз.** Contract - В OrderRepository изменяем чтение из поля status на поле order_state, оставляем запись только в поле order_state. Создаем скрипт на удаление поля status.
